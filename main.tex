...
\end{definition}
\chapter{Upper bound}
\begin{lemma}[Upper bound for dimension]\label{lem:lower_bound}
...
\end{align*} 
\todo{Will this not imply that $\dim_H(\xi^1(\partial \Gamma)) \leq h_\rho (max_p \Psi^p)$ which may be strictly smaller than $h_\rho (min_p \Psi^p) = h_\rho (F)$ ?}
\end{proof}
