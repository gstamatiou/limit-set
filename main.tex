...
To accomplish this, Sullivan generalised to all hyperbolic spaces the construction done for the hyperbolic plane by Patterson in \cite{patterson1976limit} of a $\Gamma$-quasi-equivariant measure on the limit set of hyperbolic space.
...
\begin{remark}
    ...
    \todo{Either remove references to the boundaru $\partial X$ or introduce it, and check if more hypothesis are needed for the characterisation}
\end{remark}
...
For $\Theta \subseteq \llbracket 1, d \rrbracket$ we define the space of partial flags 
...
For the shake of readability, we will often skip writing the braces around the indices of the flags and write $\mathcal F_{i,j}(\mathbb R^d)$ instead of $\mathcal F_{\{i,j\}}(\mathbb R^d)$.
...
\begin{theorem}[\cite{labourie_anosov_2006,kapovich2017anosov}]\label{thm:boundary_map}
    ...
    Then there exists an equivariant continuous map $\xi: \hat \Gamma \to \mathcal F_{p, d-p}(\mathbb R^d) $ that restricts to
    \begin{align*}
        \xi^p(\gamma) &= U_p(\rho(\gamma)) \equaldef k \cdot \mathbb R e_1 \oplus \cdots \oplus \mathbb R e_p\\
        \xi^{d-p}(\gamma) &= U_{d-p}(\rho(\gamma)) \equaldef k \cdot \mathbb R e_1 \oplus \cdots \oplus \mathbb R e_{d-p},
    \end{align*}
    for $\gamma \in \Gamma, \rho(\gamma) = k e^a l$ and where $e_1, \ldots, e_d$ is the standard orthonormal basis for the Euclidean inner product.
...
\begin{align*}
    ...
    &= 
    {\sf a}_{1,2} + \cdots + {\sf a}_{1,p + 1} + p\left(\frac{s}{p}-1\right) {\sf a_{1,p+2}} \geq\\
    &\geq
...
\begin{definition}
    ...
    Using the canonical identification $T_l \mathbb P(V) \simeq \hom (l, V/l)$, we define the density $|\Omega_{l, V}|: \left( T_l \mathbb P(V) \right)^p \to \mathbb R^+$ by
    \[
...
\todo{Add some theory on densities and show that the density on $\mathcal F_{1. p+1} (mathbb R^d)$ is $\mathbb SL(d, R)$ equivariant}
...
\begin{definition}\index{Patterson--Sullivan measure}
    ...
    a $(\Gamma, \phi)$-Patterson Sullivan measure on $\mathcal F_\Theta$ is a finite Radon measure $\mu$ such that for every $h \in H$
    \[
...
\end{theorem}
\begin{proof}
    We have already seen in \cref{lem:upper_bound} that $d_{\rho, \Gamma} \leq h_{\rho(\Gamma)}(F)$.
    ...
    There we have seen that $h_{\rho(\Gamma)}(F) \leq h_{\rho(\Gamma)}(\Psi^{d_{\rho, \Gamma} + 2})$ since $F_s \geq \Psi_s^{d_{\rho, \Gamma} + 2}$, so may as well show that $h_{\rho(\Gamma)}(\Psi^{d_{\rho, \Gamma} + 2}) \leq d_{\rho, \Gamma}$, i.e.\ that
    \[
        \sum_{\gamma \in \Gamma} e^{-\Psi^{d_{\rho, \Gamma} + 2}_s(\rho(\gamma))} < \infty
    \]
    for all $s \geq d_{\rho, \Gamma}$.
    This will follow as soon as we have shown that $h_{\rho(\Gamma)}(J_{d_{\rho, \Gamma}}) \leq 1$, since
    \begin{align*}
